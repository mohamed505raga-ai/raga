...
- **DELETE /api/products** — Query: `?id=<productId>`. Permanently removes the product from `data/store-data.json`. Uses the same GitHub env vars. Responds `200 { success: true, products }` on success. Used by the Admin Dashboard when **Orders API URL** is set (same base URL).
